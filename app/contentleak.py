...
from app.subprocessors import query_shodan, query_zoomeye, query_modat
...
def main(requestobject, doshodan=True, docensys=True, dozoome=True, dofofa=True, domodat=True):
    ...
    log.info('contentleak: body hash mmh3=%s md5=%s sha256=%s',
             body_hash['mmh3'], body_hash['md5'], body_hash['sha256'])
    # Shodan and Modat index the mmh3 of the HTML body (http.html_hash /
    # web.body_mmh3); ZoomEye indexes its md5 (body_hash). (FOFA/Censys have no
    # equivalent body-hash field.)
    if doshodan:
