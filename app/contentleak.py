...
def main(requestobject, doshodan=True, docensys=True, dozoome=True, dofofa=True, domodat=True, dovalidin=True):
    ...
    # Body-hash pivots, each on its engine's indexed form:
    #   Shodan  http.html_hash (mmh3)   ZoomEye body_hash (md5)
    #   Modat   web.html.sha256         Validin hash pivots (sha1)
    # Modat's query vocabulary differs from its response model: the body hash is
    # queryable as web.html.sha256/sha1/mmh3, NOT the response's body_sha256.
    # (FOFA/Censys have no equivalent body-hash field.)
    if doshodan:
        query_shodan('http.html_hash:' + str(body_hash['mmh3']))
    if dozoome:
        query_zoomeye('body_hash:' + body_hash['md5'])
    if domodat:
        query_modat('web.html.sha256="' + body_hash['sha256'] + '"')
    if dovalidin:
